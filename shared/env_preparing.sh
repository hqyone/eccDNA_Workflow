...
if [ ! -f "$parent_dir/$genome_name/GRCh38.p13.genome.fa" ]; then
    wget $REF_FA_URL -P "$parent_dir/$genome_name"
    gzip -d "$parent_dir/$genome_name/GRCh38.p13.genome.fa.gz"
...
if [ ! -f "$parent_dir/$genome_name/gtf/gencode.v37.chr_patch_hapl_scaff.annotation.gtf" ]; then 
    ...
    wget $REF_GTF_URL -P "$parent_dir/$genome_name/gtf"
    gzip -d "$parent_dir/$genome_name/gtf/gencode.v37.chr_patch_hapl_scaff.annotation.gtf.gz"
